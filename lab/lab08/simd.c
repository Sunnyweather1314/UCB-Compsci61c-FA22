...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
            int* original = vals + i;
            __m128i tmp = _mm_loadu_si128((__m128i *) original);
            __m128i cmp = _mm_cmpgt_epi32(tmp, _127);
            __m128i validated = _mm_and_si128(tmp, cmp);
            sum_vec = _mm_add_epi32(sum_vec, validated);
        }
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
            ...
            tmp = _mm_loadu_si128((__m128i *) (original + 4));
            cmp = _mm_cmpgt_epi32(tmp, _127);
            validated = _mm_and_si128(tmp, cmp);
            sum_vec = _mm_add_epi32(sum_vec, validated);

            tmp = _mm_loadu_si128((__m128i *) (original + 8));
            cmp = _mm_cmpgt_epi32(tmp, _127);
            validated = _mm_and_si128(tmp, cmp);
            sum_vec = _mm_add_epi32(sum_vec, validated);

            tmp = _mm_loadu_si128((__m128i *) (original + 12));
            cmp = _mm_cmpgt_epi32(tmp, _127);
            validated = _mm_and_si128(tmp, cmp);
            sum_vec = _mm_add_epi32(sum_vec, validated);
        }
        ...
        for(unsigned int i = NUM_ELEMS / 16 * 16; i < NUM_ELEMS / 4 * 4; i += 4) {
            int* original = vals + i;
            __m128i tmp = _mm_loadu_si128((__m128i *) original);
            __m128i comp = _mm_cmpgt_epi32(tmp, _127);
            __m128i validated = _mm_and_si128(tmp, comp);
            sum_vec = _mm_add_epi32(sum_vec, validated);
        }
