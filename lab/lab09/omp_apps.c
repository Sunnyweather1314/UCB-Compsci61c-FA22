...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    ...
    #pragma omp parallel 
    {
        ...
        int thread_id = omp_get_thread_num();
        chunck_num = ARRAY_SIZE / thread_num;
        for (int i = thread_id * chunck_num; i < (thread_id + 1) * chunck_num; i++)  {
            z[i] = x[i] + y[i];
        }
    ...
    for (int j = thread_num * chunck_num; j < ARRAY_SIZE; j++) {
        z[j] = x[j] + y[j];
    }
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
    double global_sum = 0.0;
    // TODO: Implement this function
    int thread_num, chunck_num;
    #pragma omp parallel 
    {
        thread_num = omp_get_num_threads();
        // printf(thread_num, "%d");
        int thread_id = omp_get_thread_num();
        chunck_num = arr_size / thread_num;
    ...
    for (int j = thread_num * chunck_num; j < arr_size; j++) {
        global_sum += x[j] * y[j];
    }
